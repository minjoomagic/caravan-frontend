...
const Header = (props) => {
  ...
  return(
    ...
    <Navbar bg={props.color} variant="dark">
      ...
      <Navbar.Brand>Hey {props.user ? capitalize(props.user.username) : null }!</Navbar.Brand>
      ...
      <Button variant="outline-light" onClick = {toggleForm}>Sell Item</Button>
      <Navbar.Brand>Search for an Item!</Navbar.Brand>
      <CategoryFilter categories={props.categories} selectHandler={props.selectHandler}/>
      <SearchBar submitHandler={props.submitHandler}/>
      <Button onClick={logOutHandler} variant="outline-light"> Log Out </Button>
    </Navbar>
    </div>
  )
}
